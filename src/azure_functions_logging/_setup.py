...
def setup_logging(
    *,
    ...
    activate_trace_context: bool = False,
) -> None:
    ...
    Args:
        ...
        activate_trace_context: When ``True``, set the process-wide default so
            ``logging_context`` and ``with_context`` attach the host's W3C trace
            context (via OpenTelemetry) — making OTel log records inherit the
            host span's ``trace_id``/``span_id``. Requires the ``[otel]`` extra;
            degrades to a silent no-op when OpenTelemetry is not installed. A
            per-call ``activate_trace_context`` argument overrides this default.
